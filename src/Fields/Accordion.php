...
class Accordion extends BasicLayout {
  ...
  public function getField($key, $expectedClass = null) {
    ...
    if (!empty($this->_pageId) && !empty($this->_parentKey)) {
      $accordionKey = self::makeFieldKey($this->_parentKey, $this->_selfKey);

      $field->loadACF($this->_pageId, $accordionKey, $key);
    }
  ...
  public function loadACF($pageId, $parentKey, $key) {
    if (!is_scalar($pageId) || !is_string($parentKey) || !is_string($key)) {
      throw new Exception($this, __METHOD__ . ': invalid use of accordion API');
    }
